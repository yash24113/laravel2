...
<center><h1>Add  Temple Package </h1>
          <br><br><br><br>
         <form action="{{route('boatt.store')}}" method="post" class="">
  @csrf
...
<tr>
	<td>{{$boatt->id}}</td>
  <td> <img src="{{asset('images/' . $boatt->image) }}"  height="40px" width="40px"/></td>
	<td>{{$boatt->package}}</td>
	<td>{{$boatt->price}}</td>
    <div class="e">
	<td><a href="{{route('boatt.edit',$boatt->id)}}" class="ee">Edit</a></td>
</div>
	<td>
	<form action="{{route('boatt.destroy',$boatt)}}" method="post">
		@csrf
		@method('DELETE')
	
	<button type="submit" onclick="return confirm('sure to delete')">Delete</button>
</form>
</td>
